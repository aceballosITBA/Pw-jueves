...
const calcPrice = (base, pack) => pack === 12 ? Math.round(base * 2 * 0.9) : pack === 24 ? Math.round(base * 4 * 0.8) : base;

export default function ProductDetail({ product, selectedPack, onPackChange, onAddToCart, onGoToCart }) {
    ...
    <section className="product-detail-panel" id="ofertas">
      <div className="detail-main">
        ...
        <div className="detail-info">
          ...
          <div className="detail-actions"><button className="buy-now" onClick={() => { onAddToCart?.(product, selectedPack); onGoToCart?.(); }}>Comprar ahora</button><button className="add-cart" onClick={() => onAddToCart?.(product, selectedPack)}>Agregar al carrito</button></div>
